package com.sml.mass.ImageSpannable;
...
public class FolderTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    /* 默认展开文字 */
    private static final String DEFAULT_UNFOLD_TEXT = "阅读更多";
    /* 默认固定行数 */
    ...
    /* 展开事件 */
    private OnFoldStateChangeListener onFoldStateChangeListener;
    ...
    private ClickableSpan clickSpan = new ClickableSpan() {
        ...
        public void onClick(View widget) {
            ...
            if (onFoldStateChangeListener != null) {
                onFoldStateChangeListener.onFoldStateChanged(mIsFold);
            }
            mHasDrawn = false;
            invalidate();
        }
    ...
    public FolderTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.FolderTextView);
        mFoldText = typedArray.getString(R.styleable.FolderTextView_foldText);
        if (null == mFoldText) {
            mFoldText = DEFAULT_FOLD_TEXT;
        }
        mUnFoldText = typedArray.getString(R.styleable.FolderTextView_unFoldText);
        if (null == mUnFoldText) {
            mUnFoldText = DEFAULT_UNFOLD_TEXT;
        }
        mFoldLine = typedArray.getInt(R.styleable.FolderTextView_foldLine, DEFAULT_FOLD_LINE);
        if (mFoldLine < 1) {
            throw new RuntimeException("foldLine must not less than 1");
        }
        mTailColor = typedArray.getColor(R.styleable.FolderTextView_tailTextColor, DEFAULT_TAIL_TEXT_COLOR);
        mCanFoldAgain = typedArray.getBoolean(R.styleable.FolderTextView_canFoldAgain, DEFAULT_CAN_FOLD_AGAIN);
    ...
    public OnFoldStateChangeListener getOnFoldStateChangeListener() {
    ...
    public void setOnFoldStateChangeListener(OnFoldStateChangeListener onFoldStateChangeListener) {
        this.onFoldStateChangeListener = onFoldStateChangeListener;
    }
     ...
     * @param text 源文本
     */
    private void updateText(SpannableString text) {
        mIsInner = true;
        if (span != null) {
            text.setSpan(span, startIndex, endIndex, spanFlag);
        }
        setText(text);
    }
    ...
    private String tailorText(String text) {
        ...
        String ret;
        if (find == 0) {
            ret = text.substring(0, mid - (endIndex - startIndex)) + DEFAULT_ELLIPSIZE + mUnFoldText;
        } else {
